...
for ($l = 7; $l<=10; $l++){
	...
	for ($i = 1; $i<=15;$i++){
			...
			if ($arr["address"] != null && $arr["name"] != null && $arr["birth_data"] != null ){
				$address = $arr["address"];
				$name = removeText($arr["name"]);
				$birth = $arr["birth_data"];
				$encode = password_hash($pass, PASSWORD_DEFAULT);
				$dir = '/img/'.$lop.'/'.trim(str_replace(' ', '_', vn_to_str($name))).'.jpg';
				$use_dir = dirname(__FILE__).$dir;
				imagejpeg($img, $use_dir);
				$sql = "INSERT INTO $lop (username, password, password_encode, Hinhanh, HovaTen, Namsinh, Gioitinh, Diachi) VALUES (
						'$token',
						'$pass',
						'$encode',
						'$dir',
